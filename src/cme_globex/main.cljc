(ns cme-globex.main
  "Kotodama WASM entrypoint for the CME Globex clean-room actor (L4) — Clojure port.
  ...
  facts (`cme_globex.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cme_globex")
(def tier "L4")
...
(def entity-specs
  [{:entity "Instrument" :plural "instruments" :id-prefix "cmeglobe_ins"
   ...
   {:entity "Execution" :plural "executions" :id-prefix "cmeglobe_exe"
    :fields [:orderId :quantity :price :executedAt]          :required [:quantity :price]
    :coerce {:quantity :float :price :float}       :refs {:orderId "Order"}}
   {:entity "MarketData"   :plural "marketdatas"   :id-prefix "cmeglobe_mar"
   ...
   {:entity "Position"    :plural "positions"    :id-prefix "cmeglobe_pos"
...
(defn emit-facts
  "EAVT facts for one record: {\"cme_globex.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cme_globex-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cme_globex-compat/ready)
